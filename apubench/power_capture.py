...
def run_quiescent(quiescent, require_samples=False, verbose=False):
    '''Pause for 'quiescent' seconds and capture power info'''

    if verbose: print("%s: Entering quiescent period=%d seconds..." % (sys.argv[0],quiescent))

    power_stats = None
    if power_capture.enabled():

        if verbose: print("%s: Starting power capture..." % sys.argv[0])

        psession_id = power_capture.start()
        if verbose: print("%s: Got session id=" % (sys.argv[0]),psession_id)

        start_time = end_time = datetime.now()
        while (end_time - start_time).total_seconds() < quiescent:
            if verbose: print("%s: waiting in quiescent mode..." % sys.argv[0])
            time.sleep(1)
            end_time = datetime.now()

        if verbose: print("%s: Quiescent wait ended." % sys.argv[0])
        power_stats = power_capture.stop(psession_id, all_stats=True)
        if verbose: print("%s: power_stats=" % sys.argv[0], power_stats)
        if require_samples and ( (type(power_stats)==type({}) and power_stats['tot_power']<=0) \
                or (type(power_stats)==type(-1) and  power_stats<=0 ) ):
            raise Exception("%s: power samples required but didn't get any." % sys.argv[0])

    bench_info = {}
    bench_info["power"] = power_stats
...
if __name__ == "__main__":
    ...
    ipmicap_ip = "127.0.0.1" # Set to your ipmicap's server ip
    ...
    print("stop")   
    stats = power_capture.stop(cid,all_stats=True)
